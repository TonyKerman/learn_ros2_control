...
2. Install the package by calling `colcon build --symlink-install --packages-select steering_chassis_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb steering_chassis_description` command.

Now, launch description test:
```
ros2 launch steering_chassis_description view_steering_chassis.launch.xml
```
or
```
ros2 launch steering_chassis_description view_steering_chassis.launch.py
```
