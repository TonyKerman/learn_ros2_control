...
2. Install the package by calling `colcon build --symlink-install --packages-select steering_wheel_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb steering_wheel_description` command.

Now, launch description test:
```
ros2 launch steering_wheel_description view_steering_wheel.launch.xml
```
or
```
ros2 launch steering_wheel_description view_steering_wheel.launch.py
```
